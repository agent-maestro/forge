// eml_asin.v -- asin(x) in Q<WIDTH-FRAC>.<FRAC> fixed-point.
//
// Status: SCAFFOLD. Implements 3-term Taylor series valid for
// x in [-0.5, 0.5]. For |x| in (0.5, 1) the series converges
// slowly -- caller should switch to the identity
//   asin(x) = pi/2 - 2*asin(sqrt((1-x)/2))
// outside the safe band.
//
//   asin(x) ~= x + x^3/6 + 3*x^5/40 + 15*x^7/336    (|x| <= 0.5)
//
// Pipeline: 4 stages.
